  ...
  if (listaProductosCarrito.length == 0) {
    ...
    for (i = 0; i < listaProductosCarrito.length; i++) {
      if (listaProductosCarrito[i][2] == theProduct["name"]) {
        listaProductosCarrito[i] = [
          listaProductosCarrito[i][0],
          listaProductosCarrito[i][1] + 1,
          listaProductosCarrito[i][2],
          listaProductosCarrito[i][3],
          listaProductosCarrito[i][4] + parseFloat(theProduct["price"]),
        ];
        yaIngresado = true;
        nItemsCarrito++;
      }
    ...
    if (!yaIngresado) {
      listaProductosCarrito.push([
        listaProductosCarrito.length + 1,
        1,
        theProduct["name"],
        theProduct["price"],
        parseFloat(theProduct["price"]),
...
function showTable() {
  ...
  for (c = 0; c < listaProductosCarrito.length; c++) {
    costoTotalCarrito =
      costoTotalCarrito + parseFloat(listaProductosCarrito[c][4]);
...
function displayCards(products) {
  ...
  for (j = 0; j < products.length; j++) {
    ...
    cardButton.onclick = function () {
      agregarProductoAcarrito(products, cardTitle.innerHTML);
    };
